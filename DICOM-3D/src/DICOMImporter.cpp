...
void DICOMImporter::ApplyTexture3D() {

	if (!loaded) {
		allVolumeData = new char[2 * width * height * numberOfTextures];
		for (int i = 0; i < numberOfTextures; i++) {
			...
			if (!imReader.Read()) {
				...
				currentBuffer = new char[image.GetBufferLength()];
				image.GetBuffer(currentBuffer);
				std::copy(currentBuffer, currentBuffer + image.GetBufferLength(), allVolumeData + i * image.GetBufferLength());
			}
		}
		loaded = true;
	}
	glTexImage3D(GL_TEXTURE_3D, 0, GL_R16, width, height, numberOfTextures, 0, GL_RED, GL_UNSIGNED_SHORT, allVolumeData);
}
